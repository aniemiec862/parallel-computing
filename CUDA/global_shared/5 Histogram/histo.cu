...
int run_histogram(int whichKernel, int BIN_COUNT)
{
    int deviceCount;
...
{   
    printf("whichKernel;BIN_COUNT;elapsedTime\n");

    int whichKernel;
	int BIN_COUNT[2] = {16, 1048576};
    int number_of_retries = 3;

    for (int i = 0; i < sizeof(BIN_COUNT) / sizeof(BIN_COUNT)[0]; i++) {
        for (whichKernel = 0; whichKernel < 2;  whichKernel++) {
		    for (int k = 0; k < number_of_retries; k++) {
	            GpuTimer timer;
                timer.Start();

                run_histogram(whichKernel, BIN_COUNT[i]);
                timer.Stop();
