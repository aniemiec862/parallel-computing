...
def drawAllSpeedupPlot():
    ...
    smallSpeedupData = (
        smallTimeData[0],
        [speedup(smallT1, entry, n + 1) for n, entry in enumerate(smallTimeData[1])]
    )

    mediumT1 = mediumTimeData[1][0]
    mediumSpeedupData = (
        mediumTimeData[0],
        [speedup(mediumT1, entry, n + 1) for n, entry in enumerate(mediumTimeData[1])]
    )

    bigT1 = bigTimeData[1][0]
    bigSpeedupData = (
        bigTimeData[0],
        [speedup(bigT1, entry, n + 1) for n, entry in enumerate(bigTimeData[1])]
    )
